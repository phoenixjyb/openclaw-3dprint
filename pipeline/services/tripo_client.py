...
async def generate_and_download(
    ...
    def _run_sync():
        with TripoClient(api_key=api_key) as client:
            log.info("Tripo3D: creating text-to-model task for: %s", prompt[:100])
            task_id = client.text_to_model(
                prompt=prompt,
            ...
            convert_task_id = client.convert_model(
                original_model_task_id=task_id,
                format="STL",
                flatten_bottom=True,
                pivot_to_center_bottom=True,
            )
            convert_task = client.wait_for_task(
                convert_task_id,
                polling_interval=poll_interval,
                timeout=poll_timeout,
                verbose=True,
            ...
            files = client.download_task_models(convert_task, output_dir)

            thumb_path = client.download_rendered_image(
            ...
            return task_id, stl_path, thumb_path or ""

    task_id, stl_path, thumb_path = await asyncio.get_event_loop().run_in_executor(
        None, _run_sync
    )
